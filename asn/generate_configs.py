...
def generate_frr_configs_extra(k=4):
    ...
    for name, cfg in confs.items():
        with open(f"config/{name}.conf", "w") as f:
            ...
            f.write("  bgp router-id 1.1.1.1\n") # 建议给每个 sw 不同的 ID，但 1.1.1.1 也能跑
            ...
            # 在 IPv4 家族中启用邻居并允许重复 AS
            for nb in cfg["neighbors"]:
                # 提取邻居 IP
                nb_ip = nb.split()[1]
                f.write(f"    neighbor {nb_ip} activate\n")
                f.write(f"    neighbor {nb_ip} allowas-in 2\n") # 核心：允许 AS 出现 2 次
